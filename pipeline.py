...
st.set_page_config(layout="wide", page_title="Accident Prediction App")
...
def predict_accident(features):
    ...
    return pipe.predict(arr)[0], pipe.predict_proba(arr)

# App UI
st.markdown("""
    <h1 style='text-align: center; color: black;'>Accident Prediction With Pipeline</h1>
...
input_data = {
    ...
    "Educational_level": st.sidebar.selectbox("Educational Level", ['None', 'Primary', 'Junior high school', 'High school', 'Above high school']),
    "Vehicle_driver_relation": st.sidebar.selectbox("Driver Relation to Vehicle", ['Owner', 'Employee', 'Unknown']),
    "Driving_experience": st.sidebar.selectbox("Driving Experience", ['No Licence', '1-2yr', '2-5yr', 'Above 10yr', 'Unknown']),
    "Type_of_vehicle": st.sidebar.selectbox("Vehicle Type", ['Automobile', 'Lorry', 'Taxi', 'Public bus', 'Long lorry']),
    "Owner_of_vehicle": st.sidebar.selectbox("Owner of Vehicle", ['Owner', 'Organization']),
    "Service_year_of_vehicle": st.sidebar.selectbox("Service Year", ['0-2yr', '2-5yr', '5-10yr', 'Above 10yr', 'Unknown']),
    ...
    "Lanes_or_Medians": st.sidebar.selectbox("Lanes", ['Undivided Two way', 'Two-way (divided with solid lines road marking)']),
    ...
    "Type_of_collision": st.sidebar.selectbox("Collision Type", ['Vehicle with vehicle', 'Collision with animals', 'Unknown']),
...
if st.sidebar.button("Predict"):
    ...
    st.success(f"\U0001F691 Predicted Severity: **{severity_map[result]}**")

    # Confidence
    st.markdown("### \U0001F50D Prediction Confidence")
    ...
    try:
        ...
        sns.barplot(x=importances[top_idx], y=[f"Feature {i}" for i in top_idx])
